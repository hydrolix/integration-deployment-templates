...
use bundle_validator::bundle_struct::Bundle;
...
const PROJ_UUID: &str = "b0956252-36a3-4c09-aad9-c7549b2fc685";  // QA environment (default)
const PROJ_UUID_SAND: &str = "469dbd34-6f06-4dfe-8fd1-9adf82123ecf";  // partnersandbox sample_project
...
async fn delete_functions_impl(
    ...
    let functions_to_delete: Option<HashSet<String>> = if let Some(b) = bundle {
        ...
        if let Ok(discovered) = hdx::discover_functions(bundle_name).await {
...
async fn delete_dictionary_files_impl(
    ...
    let file_names_to_delete: Option<HashSet<String>> = if let Some(b) = bundle {
        ...
        // Try to discover from bundle directory
        if let Ok(discovered) = hdx::discover_dictionaries(bundle_name).await {
...
async fn delete_tables_impl(
    ...
    let table_list_text = hdx::get_table_list(bearer_token, false)
        .await
    ...
    for (name, uuid) in tables {
        ...
        match hdx::delete_a_table(bearer_token, &uuid).await {
            Ok(_) => {
                println!("  ✓ Deleted: {}", name);
                deleted += 1;
            }
