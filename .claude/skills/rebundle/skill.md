...
1. **Delete everything in the bundle directory except `bundle-config.json`:**
   ```bash
...
2. **Copy everything from `.originals/`** into the bundle directory:
   ```bash
...
3. **Verify the bundle directory** contains only:
   ...
   - Raw summary SQL(s) in `summaries/` (if any)
