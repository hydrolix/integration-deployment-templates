...
**`No rows queryable in ... after 60s.`** The `--local` / `--guid` validator now verifies that `sample_data` actually lands in the test table after ingest — HTTP 2xx from `/ingest/event` does not prove the row is queryable. When the verifier polls `count() = 0` for the full window, it runs four diagnostics and embeds them in the error. The most common causes:

- **`PRIMARY TIMESTAMP STALE`** — your `sample_data` primary timestamp is older than the test table's `max_age_days` (1), so the partition was expired on arrival. Refresh the timestamp to within the last day.
- **`sample_data MISSING REQUIRED FIELDS: [...]`** — your `output_columns` declare fields that aren't keys in `sample_data`; the transform drops those rows silently. Add the missing keys.
- **`Recent ingest errors: ...`** — the transform rejected the row at parse time (datatype mismatch, bad SQL). Read the error verbatim from `system.ingest_errors`.
- **`system.parts: {total: N, active_count: 0, ...}`** — parts landed but were aged out; confirms the staleness story.
